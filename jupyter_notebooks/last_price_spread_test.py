...
def get_historical_data(tickers, start_date, end_date):
    ...
    for ticker in tickers:
        ...
        data[ticker] = stock.history(start=start_date, end=end_date, interval='1m')
...
def run():
    lower = 1
    upper = 50
    main_df = pd.read_csv(f'/home/elliott/Development/scripts/jupyter_notebooks/spreads-202308090915.csv')
    ...
    merged_df['Date'], merged_df['Time'] = merged_df['Time'].str.split(' ', 1).str
    ...
    merged_df.to_csv("first_merged_df.csv")
    tickers = merged_df['underly'].unique().tolist()
    data = get_historical_data(tickers, "2023-08-07", "2023-08-09")
    all_data = []
    ...
    result_df = merged_df.merge(historical_df, left_on=['underly', 'Date'], right_on=['Ticker', 'Date'], how='left')
