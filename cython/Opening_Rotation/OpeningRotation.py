...
from cyoptimized_OpeningRotation_funcs import assign_equity_prices_at_915_cython
...
def generate_and_execute_sql(result_df):
    ...
    query = f"""
        ...
        AND r.create_time >= timestamp '{previous_day}'
...
def main():
    ...
    original_df_array = merged_df.to_numpy()

    batch_data_df_array = prices915_df.to_numpy()
    
    merged_df = assign_equity_prices_at_915_cython(original_df_array, batch_data_df_array, date)
    merged_df = pd.DataFrame(original_df_array, columns=merged_df.columns)

    print(f'after price_at_915 assigning... in {merged_df.columns}')
