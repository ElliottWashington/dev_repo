...
def write_to_database(date):
    ...
    filename = f"average_cancel_ack_latency_{date}.csv"
...
def write_to_database(date):
    # Construct the filename of the CSV file to read from
    filename = f"avg_spread_nack_latencies_{date}.csv"
    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO average_latency_spread (date, exchange, average_difference, count) VALUES (%s, %s, %s, %s)",
                (date, exchange, avg_difference, count)
            )
            conn.commit()
