    ...
    for part in parts:
        ...
        for i in range(len(part) - 5):  # Subtract 5 to avoid running off the end of the string
            if part[i:i+2].isdigit() and part[i+2:i+4].isdigit() and part[i+4:i+6].isdigit():
                date_start = i
                break  # If a date substring couldn't be found, treat the part as a non-option
        if date_start is None:
            new_parts.append(part)
            continue # Extract underlying, date, call/put, strike, and suffix
...
@timing_decorator
def get_batch_data2_from_batch_data1(df, batch_data_df_from_get_batch_data1, current_date):
    ...
    # Filter the DataFrame based on the unique symbols in df['underly'] and the time range
    symbols = df['underly'].unique()
    batch_data_df = batch_data_df_from_get_batch_data1[
        batch_data_df_from_get_batch_data1['Symbol'].isin(symbols) &
...
def chunked_symbols_generator(symbols, chunk_size):
    it = iter(symbols)
    while True:
...
def process_chunk(original_df, batch_data_df, chunk):
    # Filter the original DataFrame by symbols in the chunk
    original_subset = original_df[original_df['underly'].isin(chunk)].copy()
    
    # Filter the batch DataFrame by symbols in the chunk
    batch_subset = batch_data_df[batch_data_df['Symbol'].isin(chunk)].sort_values(by="Timestamp")
...
@timing_decorator
def assign_closest_prices_parallel_chunked(original_df, batch_data_df, chunk_size=8):
    # List to store the results
    result_list = []
    
    # Create a ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=8) as executor:
        symbols = original_df['underly'].unique()
        chunks = chunked_symbols_generator(symbols, chunk_size)
        
        futures = {executor.submit(process_chunk, original_df, batch_data_df, chunk): chunk for chunk in chunks}
        
        for future in as_completed(futures):
            chunk = futures[future]
            try:
                processed_subset = future.result()
                result_list.append(processed_subset)
            except Exception as e:
                print(f"An exception occurred while processing chunk {chunk}: {e}")
...
def generate_and_execute_sql(result_df):
    previous_day = (datetime.now() - timedelta(days=5)).strftime('%Y-%m-%d')
    ...
    query = f"""
        ...
        AND r.create_time >= timestamp '2023-09-15'
...
def main():
    ...
    # Merge and further process the main dataframe
    merged_df = merge_and_process(main_df, date)

    # Get batch data based on the merged dataframe for a specific time range (all times)
    pricesattime_df = get_batch_data1(merged_df)

    # Extract the subset of batch data around 9:14 and 9:15
    prices915_df = get_batch_data2_from_batch_data1(merged_df, pricesattime_df, date)

    # Assign equity prices at 9:15 to the main dataframe
    assign_equity_prices_at_915(merged_df, prices915_df, date)

    # Assign closest equity prices to the main dataframe
    merged_prices_df = assign_closest_prices_parallel_chunked(merged_df, pricesattime_df)

    # Calculate various metrics and prices
    result_df = calculate_metrics_and_prices(merged_prices_df)
    
    # Generate and execute SQL in parallel to get rates and volatilities
    rates_vol_dict = generate_and_execute_sql_parallel(result_df)
    
    # Lookup rates and vols and assign them to the main dataframe
    result_df = result_df.apply(lookup_rates_and_vols, axis=1, args=(rates_vol_dict,))

    # Drop rows where rate1 is NaN
    result_df.dropna(subset=['rate1'], inplace=True)

    # Calculate Black-Scholes and Delta
