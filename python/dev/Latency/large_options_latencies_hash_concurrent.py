...
# Define a function to map the tag100 values to exchange names
def map_exchange(tag100):
    exchange_map = {
        "1": "AMEX",
        "64": "NASDAQ",
        "84": "BATS",
    ...
    return exchange_map.get(tag100, tag100)
...
def run_queries(date):
    ...
    for key in hash_table.keys():
        ...
        hash_table[key]['reply_OID'] = hash_table[key].pop('tag11_a')
        hash_table[key]['exchange'] = hash_table[key].pop('exchange')
        hash_table[key]['sent_time'] = hash_table[key].pop('cancel_52')
        hash_table[key]['reply_time'] = hash_table[key].pop('cancelack_52')
        hash_table[key]['target_comp'] = hash_table[key].pop('target_compid')
    return hash_table

def write_to_database(date):
    # Construct the filename of the CSV file to read from
    filename = f"/home/elliott/development/files/options_cancelack_2%highest_latencies_{date}_summary.csv"
    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO average_latency_option (date, exchange, count, average_difference) VALUES (%s, %s, %s, %s)",
...
def main():
    ...
    for row in table2_data:
        ...
        result_data.append({
            'OID': row['OID'],
            'sent_time': row['sent_time'],  # Updated key to 'sent_time'
            'reply_time': row['reply_time'],  # Updated key to 'reply_time'
            'reply_OID': row['reply_OID'],  # Updated key to 'reply_OID'
            'exchange': row['exchange'],
